...
Public Class base
    ...
    Public Sub conectar()
        ...
        End If
        Try
            conexion.ConnectionString = My.Settings.baseaguaConnectionString
            conexion.Open()
    ...
    Public Sub MATACONExIONES()
        Dim CONEX As OdbcDataReader
        CONEX = consultasql("SHOW PROCESSLIST")
        Dim COLE As New Collection
        Try
            While CONEX.Read
                If CONEX("COMMAND") = "Sleep" And CONEX("TIME") > 300 And (CONEX("Host").ToString().ToUpper().Contains("CAJA") Or CONEX("Host").ToString().ToUpper().Contains("PRESIDENCIA") Or CONEX("Host").ToString().ToUpper().Contains("CONNIE")) Then
