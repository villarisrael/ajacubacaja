...
Public Class Clscontrolpago
    ...
    Public totaldescuentorecargo As Double = 0
    ...
    Public tieneConvenio As Boolean
    Public fechaConvenio As Date

    Public Sub calcula(ByVal permitiranticipo As Boolean, tieneConvenio_ As Boolean, fechaConvenio_ As Date)
        tieneConvenio = tieneConvenio_
        fechaConvenio = fechaConvenio_
        variable_iva = 16
        totaldeudaconsumo = 0
                ...
                If rs(My.Settings.booleanalcantarillado) <> 0 Then
                    ...
                    If rs(My.Settings.booleancobrarfijoalcantarillado) <> 0 Then ' si va cobrar fijo alcantarillado
                        ...
                        For i = 1 To consumo.collection.Count
                            ...
                            objetoalca.Total = objetlectura.Total * 0.25
                            '''
                            '''''''''''''''''''''''
                            objetoalca.Totalcondescuento = objetoalca.Total 'rs(My.Settings.campoalcantarillo)
    ...
    Public Sub llenaotrosn()
        Dim DATOS As OdbcDataReader
        Dim x As New base
        If tieneConvenio = False Then
            'Esta Consulta genero error con el lcontrato 4910, no cargaba la multa por la condición de la fecha 12/04/2022

            DATOS = x.consultasql("SELECT  CANTIDAD, CONCEPTO, subtotRESTA AS MONTO, conceptoscxc.id_concepto AS CLAVE , otrosconceptos.CLAVE AS CLAVEMOV,unidadsat, clavesat,aplicaiva FROM otrosconceptos,conceptoscxc WHERE otrosconceptos.CUENTA=" & cuenta & " AND PAGADO=0 AND otrosconceptos.ESTADO<>'CANCELADO' AND conceptoscxc.id_concepto=otrosconceptos.id_concepto AND vencimiento <= '" & UnixDateFormat(Now) & "'")

            'DATOS = x.consultasql("SELECT  CANTIDAD, CONCEPTO, subtotRESTA AS MONTO, conceptoscxc.id_concepto AS CLAVE , otrosconceptos.CLAVE AS CLAVEMOV,unidadsat, clavesat,aplicaiva FROM otrosconceptos,conceptoscxc WHERE otrosconceptos.CUENTA=" & cuenta & " AND PAGADO=0 AND otrosconceptos.ESTADO<>'CANCELADO' AND conceptoscxc.id_concepto=otrosconceptos.id_concepto")

            'DATOS = x.consultasql("SELECT  CANTIDAD, CONCEPTO, subtotRESTA AS MONTO, conceptoscxc.id_concepto AS CLAVE , otrosconceptos.CLAVE AS CLAVEMOV,unidadsat, clavesat,aplicaiva FROM otrosconceptos,conceptoscxc WHERE otrosconceptos.CUENTA=" & cuenta & " AND PAGADO=0 AND otrosconceptos.ESTADO<>'CANCELADO' AND conceptoscxc.id_concepto=otrosconceptos.id_concepto AND '" & UnixDateFormat(Now) & "' <= vencimiento")
        Else
            DATOS = x.consultasql("SELECT  CANTIDAD, CONCEPTO, subtotRESTA AS MONTO, conceptoscxc.id_concepto AS CLAVE , otrosconceptos.CLAVE AS CLAVEMOV,unidadsat, clavesat,aplicaiva FROM otrosconceptos,conceptoscxc WHERE otrosconceptos.CUENTA=" & cuenta & " AND PAGADO=0 AND otrosconceptos.ESTADO<>'CANCELADO' AND conceptoscxc.id_concepto=otrosconceptos.id_concepto AND vencimiento <= '" & UnixDateFormat(fechaConvenio) & "'")
