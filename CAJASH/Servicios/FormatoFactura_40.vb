...
Public Class FormatoFactura_40
    ...
    Public Sub GenerarPDFFactura_CFDI4(serieFacturaP As String, folioFacturaP As Integer, cuentaUsuarioP As String, reimpresionPDF As Boolean, tipoUsuarioP As String, enviarMail As Boolean)


        Dim cadenaOriginalCertificado As String = ""
        Dim tipoFactura As Short = Short.Parse(tipoUsuarioP)


        'Dim tipoFactura = ConvertirTipoUsuario(tipoUsuarioP)
        ...
        Try
            ...
            If tipoFactura = 1 Or tipoFactura = 2 Or tipoFactura = 3 Then


                datosFiscalesUsuario = ObtenerDatosFiscales(datosCFDI40_XML.Receptor.Nombre, cuentaUsuarioP, tipoFactura)

            ElseIf tipoFactura = 4 Then

                datosFiscalesUsuario = ObtenerDatosFacturaPeriodo(datosCFDI40_XML.Receptor.Nombre)
            ...
            If ValidarDatosUsuario(datosCFDI40_XML.Receptor.Nombre, tipoFactura, datosFiscalesUsuario) Then


                Dim pdfDoc As New Document(iTextSharp.text.PageSize.LETTER, 15.0F, 15.0F, 30.0F, 30.0F)


                Dim rutasDocumento = CrearCarpetas(pdfDoc, reimpresionPDF, serieFacturaP, folioFacturaP, datosCFDI40_XML.Receptor.Nombre.Trim(), tipoUsuarioP)

                Dim rutaDocumento As String = rutasDocumento.Item1
                Dim rutaDocumentoNombreUsuario As String = rutasDocumento.Item2
                Dim rutaDocumentoXML As String = rutasDocumento.Item3

                Dim colordefinido = New Clscolorreporte()
                colordefinido.ClsColoresReporte(My.Settings.colorfactura)

                Dim pdfWriter As PdfWriter = PdfWriter.GetInstance(pdfDoc, New FileStream(rutaDocumento, FileMode.Create))
                Dim pdfWriterNombreUsuario As PdfWriter = PdfWriter.GetInstance(pdfDoc, New FileStream(rutaDocumentoNombreUsuario, FileMode.Create))
                ...
                'AgregarLogoMarcaAgua(pdfDoc, 0.5)
                ...
                Dim widths As Single() = New Single() {120.0F, 440.0F, 300.0F}
                ...
                If tipoFactura = 4 Then
                    ...
                    Dim ColDCFP = New PdfPCell(New Phrase("", Font8Normal))
                    ColDCFP.Border = 0
                    ColDCFP.HorizontalAlignment = PdfPCell.ALIGN_RIGHT
                    Table2.AddCell(ColDCFP)


                    Dim ColDC11 = New PdfPCell(New Phrase("", Font8Normal))
                    ColDC11.Border = 0
                    ...
                    Dim ColDCMeP = New PdfPCell(New Phrase("", Font8Normal))
                    ColDCMeP.Border = 0
                    ColDCMeP.HorizontalAlignment = PdfPCell.ALIGN_RIGHT
                    Table2.AddCell(ColDCMeP)

                    Dim ColDCMeP2 = New PdfPCell(New Phrase("", Font8Normal))
                    ColDCMeP2.Border = 0
                ...
                Dim ColDCUsoRegimenFiscal = New PdfPCell(New Phrase("Regimen Fiscal ", Font8Normal))
                ColDCUsoRegimenFiscal.Border = 0
                ColDCUsoRegimenFiscal.HorizontalAlignment = PdfPCell.ALIGN_RIGHT
                Table2.AddCell(ColDCUsoRegimenFiscal)

                Dim ColDCUsoRegimenFiscal2 = New PdfPCell(New Phrase(New decodificadorSAT().getRegimeng(datosCFDI40_XML.Receptor.RegimenFiscalReceptor), Font8Normal))
                ColDCUsoRegimenFiscal2.Border = 0
                ColDCUsoRegimenFiscal2.HorizontalAlignment = PdfPCell.ALIGN_RIGHT
                Table2.AddCell(ColDCUsoRegimenFiscal2)
                ...
                If tipoFactura = 4 Then
                    ...
                    Dim observacionFacturaPeriodo As String = String.Empty

                    observacionFacturaPeriodo = obtenerCampo($"SELECT OBSERVACION FROM ENCFAC WHERE SERIE = '{datosCFDI40_XML.Serie}' AND NUMERO = {datosCFDI40_XML.Folio}", "OBSERVACION")

                    Col51 = New PdfPCell(New Phrase($"{observacionFacturaPeriodo}", Font8Normal))
                    Col51.Border = 0
                ...
                Dim subTotalFactura As Decimal = datosCFDI40_XML.SubTotal

                Col74 = New PdfPCell(New Phrase(subTotalFactura.ToString("C"), Font9))
                Col74.Border = 1
                ...
                Dim ivaGeneralFactura As Decimal = datosCFDI40_XML.Impuestos.TotalImpuestosTrasladados

                Col74 = New PdfPCell(New Phrase(ivaGeneralFactura.ToString("C"), Font9Bold))
                Col74.Border = 0
                Col74.HorizontalAlignment = PdfPCell.ALIGN_RIGHT
                Table7.AddCell(Col74)
                ...
                Dim totalFactura As Decimal = datosCFDI40_XML.Total

                Col74 = New PdfPCell(New Phrase(totalFactura.ToString("C"), Font9))
                Col74.Border = 1
                Col74.HorizontalAlignment = PdfPCell.ALIGN_RIGHT
                ...
                If enviarMail = True Then
                    ...
                    EnviarDocumentoCorreo(rutaDocumentoXML, rutaDocumento, cuentaUsuarioP, tipoUsuarioP)
    ...
    Public Function CrearCarpetas(pdfDocumento As Document, reimpresionArchivo As Boolean, serieDocumento As String, folioDocumento As Int16, nombreUsuario As String, tipoUduario As String) As (String, String, String)
        ...
        Dim tipoUsuarioFactura As Short = 1

        'tipoUsuarioFactura = ConvertirTipoUsuarioInverso($"{tipoUduario}")
        ...
        Try
            ...
            If reimpresionArchivo = True Then
                ...
                If Not My.Computer.FileSystem.DirectoryExists(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\FacturasReimpresas\" & Year(Now) & acompletacero(Month(Now).ToString(), 2).Trim & "\" & nombreUsuario) Then

                    My.Computer.FileSystem.CreateDirectory(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\FacturasReimpresas\" & Year(Now) & acompletacero(Month(Now).ToString(), 2).Trim & "\" & nombreUsuario)

                End If


                cadenaFolder = (Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\FacturasReimpresas\" & Year(Now) & acompletacero(Month(Now).ToString(), 2).Trim)

                cadenaFolderNombreUsuario = (Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\FacturasReimpresas\" & Year(Now) & acompletacero(Month(Now).ToString(), 2).Trim & "\" & nombreUsuario)

                'rutaPDF = $"{cadenaArchivoReimpreso}\Factura_{serieDocumento}{folioDocumento}.pdf"
            ...
            Else
                ...
                If Not My.Computer.FileSystem.DirectoryExists(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\Facturas\" & Year(Now) & acompletacero(Month(Now).ToString(), 2).Trim & "\" & nombreUsuario) Then

                    My.Computer.FileSystem.CreateDirectory(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\Facturas\" & Year(Now) & acompletacero(Month(Now).ToString(), 2).Trim & "\" & nombreUsuario)

                End If

                cadenaFolder = (Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\Facturas\" & Year(Now) & acompletacero(Month(Now).ToString(), 2).Trim)

                cadenaFolderNombreUsuario = (Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\Facturas\" & Year(Now) & acompletacero(Month(Now).ToString(), 2).Trim & "\" & nombreUsuario)
            ...
            rutaPDF = $"{cadenaFolder}\Factura_{serieDocumento}{folioDocumento}.pdf"

            rutaPDFNombre = $"{cadenaFolderNombreUsuario}\Factura_{serieDocumento}{folioDocumento}.pdf"

            cadenaRutaXML = CrearArchivoXML(serieDocumento, folioDocumento, cadenaFolder)
            CrearArchivoXML(serieDocumento, folioDocumento, cadenaFolderNombreUsuario)
        ...
        Return (rutaPDF, rutaPDFNombre, cadenaRutaXML)

    End Function

    Public Function CrearArchivoXML(serieFacturaP As String, folioFacturaP As String, rutaDirectorio As String)


        Dim cadenaXML = (rutaDirectorio & "\XMLFactura_" & serieFacturaP & folioFacturaP & ".xml").Trim

        Try


            Dim fs As FileStream = File.Create(cadenaXML)

            Dim CFDI As String = obtenerCampo($"SELECT CFDI FROM ENCFAC WHERE SERIE = '{serieFacturaP}' AND NUMERO = {folioFacturaP}", "CFDI")


            Dim info As Byte() = New UTF8Encoding(True).GetBytes(CFDI.TrimStart().TrimEnd())
        ...
        Catch ex As Exception
            ...
            MessageBox.Show($"Ocurrio un error al obtener el CFDI de la factura, contacta a soporte del sistema: {ex.ToString()}")
    ...
    Public Function ValidarDatosUsuario(nombreXMLP As String, tipoUsuarioP As String, datosFiscalesObtenidos As DatosFiscalesUsuario) As Boolean

        ' Verificar si el objeto datosFiscalesObtenidos no es nulo para evitar errores
        If datosFiscalesObtenidos Is Nothing Then
            Return False
        End If

        ' Comparar los valores con los del objeto
        If nombreXMLP = datosFiscalesObtenidos.RazonSocial AndAlso tipoUsuarioP = ConvertirTipoUsuario(datosFiscalesObtenidos.tipoUsuario) Then
            Return True
        End If
    ...
    Public Sub EnviarDocumentoCorreo(rutaDocumentoXML As String, rutaDocumentoPDF As String, cuentaUsuario As Integer, tipoUsuario As String)
        ...
        Try
            ...
            Dim datosf As OdbcDataReader = ConsultaSql("SELECT MAILDEENVIO FROM datosfiscales WHERE CUENTA=" & cuentaUsuario & " AND TIPO='" & tipoUsuario & "'").ExecuteReader
        ...
        Catch ex As Exception
            ...
            MessageBox.Show($"NO EXISTE MAIL DE ENVIÓ: {ex.ToString()}")

        End Try


        resultado = InputBox(mensaje, titulo, resultado)
        ...
        Try
            ...
            mail.Attachments.Add(New Attachment(rutaDocumentoPDF))
            mail.Attachments.Add(New Attachment(rutaDocumentoXML))
